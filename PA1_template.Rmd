...
```{r create_histogram_of_daily_steps}
library("ggplot2")
...
theMean <- paste("Mean daily steps: ", the_total_mean)
theMean
...
theMedian <- paste("Median daily steps: ", the_total_median)
theMedian
```
...
string1 <- toString(val1)
string2 <- toString(val2)

line_one <- paste("The daily five (5) minute interval containing the maximum number of steps: ", string1)
line_two <- paste("The average number of steps in this daily five (5) minute interval: ", string2)

cat(line_one, line_two, sep = "\n")
...
paste("Number of rows containing missing data ('NA') step values: ", num_na_rows)
...
```{r histogram_of_steps_with_imputed_NA_values}
...
theMDMean <- paste("Mean daily steps (NA values imputed): ", the_total_md_mean)
theMDMean
```
...
theMDMedian <- paste("Median daily steps (NA values imputed): ", the_total_md_median)
theMDMedian
...
```{r line_plot_daily_activity_panels_by_weekday_weekend}
library("ggplot2")
